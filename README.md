...
- `tools/release_readiness.py` 已作为离线汇总入口：不启动前后端、不依赖 War Thunder，只聚合 logic tests、pytest、RC 文档审计、free-text gate、replay gate、deferred HUD gate、proximity gate、V2 readiness summary、synthetic replay、可选 plugin check 和本地样本报告；`release_scope` 会直接列出 `ship_status`、free-text 真实播报 blocker、样本未证明项和下一步动作，`handoff` / `handoff_status` 会合并 v1 发布状态与 V2 live evidence 缺口；通过后再进入最后一轮真机 smoke。
