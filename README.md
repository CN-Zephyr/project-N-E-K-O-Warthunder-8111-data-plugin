...
- `tools/release_readiness.py` 已作为离线汇总入口：不启动前后端、不依赖 War Thunder，默认只聚合 logic tests、pytest、RC 文档审计、free-text gate、replay gate、deferred HUD gate、proximity gate、V2 readiness summary、V2 release matrix、final smoke packet、synthetic replay 和可选 plugin check；需要把本地大样本报告也纳入时显式加 `--include-local-sample`。`release_scope` 会直接列出 `ship_status`、free-text 真实播报 blocker、样本未证明项和下一步动作，`handoff` / `handoff_status` 会合并 v1 发布状态与 V2 live evidence 缺口；通过后再进入最后一轮真机 smoke。
- `T-Observe` 已接入 Hosted UI `observe` context：普通模式保留最近一次事件/决策/输出摘要，debug 模式才返回内存 ring buffer timeline。
- `T-Output` 已在真实 `push_message` 前接入输出背压：`output_backpressure_seconds` 窗口内压住同优先级或更低优先级事件，避免主机回复队列堆积；更高优先级事件仍可通过。真实战场事件 push 会带统一 `coalesce_key=neko_warthunder:battle_event`、事件年龄 / 过期时间 metadata（`event_age_seconds` / `event_expires_at`）和可解析到的 `target_lanlan`，让宿主队列中尚未释放的旧战场 cue 有机会被最新事件替换，并方便真机排查 fallback session / 晚播来源；`output_event_max_age_seconds` 默认 8s，会在真实 push 前丢弃已过期的旧事件，减少死亡后补播旧低空/超速等晚回复。
...
优先顺序：
1. L9 下一轮真机先复测机场起飞/复活阶段：优先确认 `radio_altitude_m` 可用；雷达高度 `<=10m` 后进入滑跑保护，`>=40m` 后解除。保护期内不应播 `low_alt_danger`，且贴地滑跑阶段不应播 `overspeed`；失速、死亡等关键事件仍应能触发。同时观察真实开口时 `coalesce_key` / `event_expired` / `event_age_seconds` / `target_lanlan` 是否减少旧低空/超速提示晚到，并确认没有走宿主 fallback session。
