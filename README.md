...
- V2 接近感知已接入 `proximity.events` / `situation`：按 id 去重后生成 `enemy_nearby`、`air_threat_nearby`、`enemy_on_six` 三类安全 generic 事件；普通接近在 COMBAT_STRESS 下被压住，空中威胁/后方威胁可进入队列，critical 安全事件仍优先。Hosted UI context / 面板暴露安全态势摘要，不暴露 raw 文本。
...
- `tools/proximity_gate.py` 已纳入离线 preflight，作为 V2 接近感知发布门禁：合成 proximity DTO 后，验证 Detector / Arbiter / Dispatcher / `push_message.parts[].text` 的安全泛化输出和门控关系。
- `tools/release_readiness.py` 已作为离线汇总入口：不启动前后端、不依赖 War Thunder，只聚合 logic tests、pytest、free-text gate、replay gate、proximity gate、synthetic replay、可选 plugin check 和本地样本报告；通过后再进入最后一轮真机 smoke。
...
- L9 已接入起飞/滑跑雷达高度保护、真实战场事件队列 coalescing、事件过期丢弃和 Hosted UI 中文化；V2 proximity awareness 已完成离线 gate。下一轮真机统一复测机场起飞/复活、后方/六点钟 proximity 样本、旧提示替换/过期丢弃、replay 与 free-text dry_run 安全。
- recovery 暂缓。
