...
- `V2 Proximity Awareness` is implemented for the non-live scope. The plugin now consumes data-layer `proximity.events` / `situation`, emits safe generic `enemy_nearby`, `air_threat_nearby`, and `enemy_on_six` events, gates them below critical safety events, exposes a safe Hosted UI awareness summary, and validates the path with `tools/proximity_gate.py`.
...
- Logic self-check currently passes: `192/192`.
...
- `tests/run_logic_tests.py` is the no-host logic self-check and should report `192/192 passed`.
...
1. In the next unified live pass, verify V2 proximity awareness on real data: confirm `proximity.events` are visible, `air_threat_nearby` can be produced from air proximity, and a rear / six-o'clock sample can be captured for `enemy_on_six`. The 2026-06-20 local sample already contains 4615 proximity air events and 9970 situation frames, but no rear-threat sample.
