...
- Logic self-check currently passes: `180/180`.
...
- `tools/release_readiness.py --run` is the no-host v1 RC gate aggregator. It runs deterministic checks and returns `ready_for_final_live_smoke` only when the branch is ready for the final focused live smoke.
- `tools/preflight.py --run` also runs the free-text and replay release gates, plugin check, synthetic replay, local sample replay, the offline readiness report, and the live test plan when the relevant local paths exist. Use `--report-output <path>` to save the Markdown report; parent directories are created automatically. The printed preflight plan points local sample replay users to `session_summary`, the Markdown / JSON report, and the live operation plan as review entries.
- `tests/run_logic_tests.py` is the no-host logic self-check and should report `180/180 passed`.
