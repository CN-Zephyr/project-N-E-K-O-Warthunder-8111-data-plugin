...
- Logic self-check currently passes: `154/154`.
...
- 2026-06-24 live `dry_run=false` testing showed the plugin can push events quickly while the host reply may arrive late and mix older event context. Mitigations now include plugin-side real-output backpressure plus host proactive-queue coalescing via `coalesce_key`; the next live test should verify they reduce stale queued replies without blocking critical interrupts.
...
- `tests/run_logic_tests.py` is the no-host logic self-check and should report `154/154 passed`.
...
1. In the next L9 live pass, verify airport spawn/takeoff behavior: `low_alt_danger` should be suppressed during the 45s takeoff grace, while stall, death, overspeed, and other critical events still pass. Also verify real-output coalescing reduces stale low-altitude / overspeed replies after newer death or critical events.
