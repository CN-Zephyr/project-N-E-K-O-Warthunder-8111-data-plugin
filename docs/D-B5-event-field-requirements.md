...
| `air_threat_nearby` | `proximity.events[].is_air == true` | V2：按 id 去重；可在 IN_FLIGHT / COMBAT_STRESS 下提示 | distance_m, compass, clock |
| `enemy_on_six` | `proximity.events[].clock in 5/6/7` 或后向 `relative_deg` | V2：低置信后方威胁，不等同完整尾随判定 | distance_m, clock, relative_deg |
| `tailing_risk` | 连续近距离后方 `proximity.events[]` | V2+：保守持续后方威胁；不是完整 `being_tailed` | distance_m, clock, relative_deg |
...
“敌机接近”已在 V2 通过数据层聚合后的 `proximity.events` 接入；保守的持续后方威胁 `tailing_risk` 已通过短窗连续近距离后方事件接入。真正的高置信 `being_tailed` 仍暂缓，因为它需要更可靠的相对速度、持续性与误判控制。
...
- 当前实现：插件不直接消费 `/map_obj.json` / `/map_info.json`，只消费数据层在 `/api/telemetry.proximity.events[]` 中给出的 edge 事实。
- 去重：按 `id` 单调去重；同一 proximity id 不重复播。
- 输出：只使用 safe metadata（距离、方位、钟点、类别），不复读 raw 文本或玩家名。
- 事件拆分：普通目标为 `enemy_nearby`，空中目标为 `air_threat_nearby`，后向钟点 / 相对角度为 `enemy_on_six`，连续近距离后方事件保守升级为 `tailing_risk`。
...
- 样本状态：2026-06-20 样本已有 4615 条空中 proximity、9970 帧 situation 和 3253 条 ground target item，但无后方 / 六点钟样本。
...
- 已完成：`tailing_risk` 作为保守升级事件，只在短窗内连续出现近距离后方 `proximity.events` 时触发。它只说明“后方威胁持续接近”，不声明完整尾随判定。
