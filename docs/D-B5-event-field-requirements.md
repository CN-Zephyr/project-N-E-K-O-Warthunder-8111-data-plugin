...
“敌机接近”已在 V2 通过数据层聚合后的 `proximity.events` 接入；真正的“被敌机尾随”仍暂缓，因为它需要更可靠的相对速度、持续性与误判控制。
...
- 事件拆分：普通目标为 `enemy_nearby`，空中目标为 `air_threat_nearby`，后向钟点 / 相对角度为 `enemy_on_six`。
- 门控：`enemy_nearby` 是低优先级，COMBAT_STRESS 下抑制；`air_threat_nearby` / `enemy_on_six` 可进入 IN_FLIGHT / COMBAT_STRESS 队列；CRITICAL_RISK / DEAD / SPAWNING 仍按 Arbiter 压制。
...
### D3 被敌机尾随 `being_tailed` — v2+
- 需要：敌我相对位置 + 速度矢量 + 持续性 → map 数据。即便有 map，误判率也极高，本就 v2+。
- 结论：继续暂缓。当前 `enemy_on_six` 只是“后方威胁接近”的低置信提醒，不声明完整尾随判定，也不使用 `being_tailed` 事件。
