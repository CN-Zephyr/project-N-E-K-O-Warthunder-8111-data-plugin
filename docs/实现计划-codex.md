...
- 逻辑自检以 `uv run python tests/run_logic_tests.py` 的 `163/163 passed` 为准。
...
- 不要沿用旧的 pre-T-Safety / pre-identity / pre-T-Output / pre-T-Kill-Coalesce / pre-L8 / pre-L9-takeoff-grace / pre-output-coalescing / pre-event-expiry / pre-T-UI2 / pre-deferred-hud-notice 测试数量；当前逻辑自检应以 `163/163 passed` 为准。
