...
- 逻辑自检以 `uv run python tests/run_logic_tests.py` 的 `151/151 passed` 为准。
...
- 不要沿用旧的 pre-T-Safety / pre-identity / pre-T-Output / pre-T-Kill-Coalesce / pre-L8 测试数量；当前逻辑自检应以 `151/151 passed` 为准。
